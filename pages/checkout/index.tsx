...
import { CreditCard, Lock, Loader2, ShoppingCart, AlertCircle } from 'lucide-react';
...
const CheckoutPage = () => {
  ...
  const InputField = ({ name, label, type = 'text', required = true, error, ...props }: any) => (
    <div>
      ...
      <input
        ...
        {...props}
      />
      {error && <p className="text-sm text-red-500 mt-1 flex items-center"><AlertCircle size={14} className="mr-1" />{error}</p>}
    </div>
  );
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-16">
            <div className="lg:col-span-7">
              <div className="bg-white dark:bg-gray-800/50 rounded-2xl p-6 md:p-8 shadow-lg">
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                  <InputField name="fullName" label="Full Name" error={formErrors.fullName} />
                  <div className="grid md:grid-cols-2 gap-6">
                    <InputField name="email" label="Email Address" type="email" error={formErrors.email} />
                    <InputField name="phone" label="Phone Number" type="tel" error={formErrors.phone} />
                  </div>
                  <hr className="dark:border-gray-700" />
                  <InputField name="addressLine1" label="Address" error={formErrors.addressLine1} />
                  <div className="grid md:grid-cols-3 gap-6">
                    <InputField name="city" label="City" error={formErrors.city} />
                    <InputField name="postalCode" label="Postal Code" error={formErrors.postalCode} />
                    <div>
                  ...
                  <hr className="dark:border-gray-700" />
                  <div>
                    <h3 className="text-lg font-medium text-gray-800 dark:text-gray-200 mb-2">Payment Method</h3>
                    <div className="bg-gray-100 dark:bg-gray-800 p-4 rounded-lg border border-gray-200 dark:border-gray-700">
                      <div className="flex items-center space-x-3">
                        <CreditCard className="w-6 h-6 text-indigo-500" />
                        <span className="font-semibold text-gray-700 dark:text-gray-300">Cash on Delivery</span>
                      </div>
                    </div>
                  </div>
                  <button
            ...
            <div className="lg:col-span-5">
              <div className="bg-white dark:bg-gray-800/50 rounded-2xl p-6 md:p-8 shadow-lg sticky top-24">
                <h2 className="text-2xl font-bold mb-6 text-gray-800 dark:text-gray-100">Order Summary</h2>
                <div className="space-y-4 mb-6">
                  {cartItems.map(item => (
                    <div key={item.id} className="flex items-center gap-4">
                      ...
                      <p className="font-semibold text-gray-800 dark:text-gray-200">${(item.price * item.quantity).toFixed(2)}</p>
                    </div>
                  ))}
                </div>
